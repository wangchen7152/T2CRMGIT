...
class Connection(pymysql.connections.Connection):
    ...
    def __exit__(self, exc, value, traceback):
        ...
        if self._pool:
            if not exc or exc in self._reusable_expection:
                ...
                self._pool.put_connection(
                    self._recreate(*self.args, **self.kwargs))
                self._pool = None
                try:
                    self.close()
                    logger.warning(
                        "Close not reusable connection from pool({}) caused by {}".format(
    ...
    def _recreate(self, *args, **kwargs):
        ...
        logger.debug('Create new connection due to pool({}) lacking'.format(
            self._pool.name))
        return conn
    ...
    def execute_query(self, query, args=(), dictcursor=True, return_one=False,
                      exec_many=False):
        """
        ...
        """
        with self:
            cur = self.cursor() if not dictcursor else self.cursor(
...
class ConnectionPool:
    ...
    def get_connection(self, timeout=1, retry_num=1):
        ...
        try:
            conn = self._pool.get(
                timeout=timeout) if timeout > 0 else self._pool.get_nowait()
            ...
            if retry_num > 0:
                ...
                logger.debug(
                    'Retry get connection from pool({}), the {} times'.format(
                        self.name,
    ...
    def put_connection(self, conn):
        ...
        try:
            ...
            logger.warning(
                "Put connection to pool({}) error, pool is full, size:{}".format(
                    self.name, self.size()))
